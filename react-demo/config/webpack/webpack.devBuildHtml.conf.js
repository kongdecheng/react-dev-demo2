...
entryBuild.map((data) => {
    fs.writeFile(webpackFile.devDirectory + '/' + data.name + '.html',
        htmlCont.replace('js/key.js', 'js/' + data.name + '.js')
            .replace('<%= htmlWebpackPlugin.options.title %>', webpackComConf.titleFun(data.name, data.title))
            .replace('<%= htmlWebpackPlugin.options.keywords %>', webpackComConf.keyFun(data.name, data.keywords))
            .replace('<%= htmlWebpackPlugin.options.description %>', webpackComConf.descriptionFun(data.name, data.description)),
        'utf8',
        function (err) {
            if (err) {
